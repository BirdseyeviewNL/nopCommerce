...
namespace Nop.Core.Caching
{
    ...
    public class DistributedMemoryCacheManager : ICacheMessageQueueProvider, IDistributedCacheManager
    {
        ...
        {
            cacheTime = (cacheTime ?? NopCachingDefaults.CacheTime);

            // Do not cache.
            if (cacheTime.Value <= 0)
            {
                if (_concurrentDictionary.TryGetValue(key, out var result))
                {
                    if (!result.Expired)
                    {
            ...
            CacheEntry entry = _concurrentDictionary.AddOrUpdate(key,
                // add delegate
                (k) =>
                {
                    var result = acquire();
        ...
        /// <param name="cacheTime">Cache time in minutes</param>
        /// <param name="utcDateTime">The utc date time for the cache entry.</param>
        public void Set(string key, object data, int cacheTime, DateTime? utcDateTime)
        {
            if (cacheTime <= 0)
                return;

            _concurrentDictionary.AddOrUpdate(key,
                // add delegate
                (k) =>
                {
                    ...
                    if (!utcDateTime.HasValue && IsSerializable(key, data))
                    {
                        CacheMessageQueue.Enqueue(new CacheMessage()
                        {
                            Operation = nameof(Set),
